...
def python_bin() -> str:
    ...
    - Frozen (PyInstaller): use system 'python' to avoid recursively calling the exe.
      Requires Python available in PATH.
    - Normal: use current interpreter.
    """
...
BACKEND_CMD = [python_bin(), "-m", "uvicorn", "app.main:app", "--port", "6789"]
...
def main():
    ...
    if is_port_in_use(6790):
        ...
        print("Open http://localhost:6790 if needed.")
    else:
    ...
    # If nothing was started (both already running), exit now.
    if backend_proc is None and frontend_proc is None:
        print("Backend and frontend already running. Exiting launcher.")
        return

    # One-shot: exit after starting services. They will keep running.
    print("Services started. Launcher will exit; processes keep running.")
