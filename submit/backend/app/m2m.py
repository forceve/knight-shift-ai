...
def _make_engine_instance(level: AILevel):
...
def _run_match_process(
...
) -> tuple[Optional[PlayerColor], Optional[str], List[str], str, str]:
    """
    Process-safe match runner; returns winner, result_reason, move_history, final_fen, status.
    """
    ...
    white_engine_instance = _make_engine_instance(white_engine)
    black_engine_instance = _make_engine_instance(black_engine)
    ...
    return winner, result_reason, move_history, board.fen(), status
...
class M2MController:
    def __init__(self, history: HistoryStore):
        ...
        # Test queue: stores (test_id, start_function) tuples
        self._test_queue: deque = deque()
        self._running_test_id: Optional[str] = None
    ...
    def _calculate_max_workers(self, white_engine: AILevel, black_engine: AILevel) -> int:
        ...
        All engines except level1 use 13 workers.
        """
        # If either engine is level1, use original logic
        if white_engine == AILevel.LEVEL1 or black_engine == AILevel.LEVEL1:
        ...
        # All other engines (except level1) use 13 workers
        return 13
    ...
    def _run_match_sync(self, match_id: str, white_engine: AILevel, black_engine: AILevel, max_moves: int, start_fen: Optional[str], time_limit_white: Optional[float], time_limit_black: Optional[float]):
        ...
        # Fresh instances per match to avoid shared state across threads/processes
    ...
    def _check_and_start_next_test(self):
        """Check if there's a test in queue and start it if no test is running."""
        with self.lock:
            if self._running_test_id is None and self._test_queue:
                test_id, start_func = self._test_queue.popleft()
                self._running_test_id = test_id
                test = self.tests[test_id]
                test.status = "running"
                self._persist_test(test)
                # Start the test in a new thread
                threading.Thread(target=start_func, daemon=True).start()
    ...
    def _run_batch_worker(self, test_id: str, white_engine: AILevel, black_engine: AILevel, games: int, swap_colors: bool, max_moves: int, start_fen: Optional[str], time_limit_white: Optional[float], time_limit_black: Optional[float]):
        ...
        def schedule_args(game_idx: int):
            ...
            return {
                ...
                "w_engine": w_engine,
                "b_engine": b_engine,
                "tl_w": time_limit_white if not swap else time_limit_black,
                "tl_b": time_limit_black if not swap else time_limit_white,
            }

        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = {}
            for idx in range(games):
                params = schedule_args(idx)
                # Create placeholder match before scheduling
                placeholder = M2MMatch(
                    match_id=params["match_id"],
                    white_engine=params["w_engine"],
                    black_engine=params["b_engine"],
                    status="running",
                    winner=None,
                    result_reason=None,
                    move_history=[],
                    final_fen=start_fen or chess.STARTING_FEN,
                    created_at=dt.datetime.utcnow().isoformat(),
                    time_limit_white=params["tl_w"],
                    time_limit_black=params["tl_b"],
                )
                with self.lock:
                    self.matches[params["match_id"]] = placeholder
            ...
            for future in as_completed(futures):
                ...
                match_id = params["match_id"]
                try:
                    winner, result_reason, move_history, final_fen, status = future.result()
                    with self.lock:
                        match = self.matches[match_id]
                        match.status = status
                        match.winner = winner
        ...
        with self.lock:
            self._persist_test(batch)
            # Mark as no longer running and start next test
            if self._running_test_id == test_id:
                self._running_test_id = None
        # Check queue outside lock to avoid deadlock
        self._check_and_start_next_test()
    ...
    def run_batch(self, white_engine: AILevel, black_engine: AILevel, games: int, swap_colors: bool, max_moves: int, start_fen: Optional[str] = None, time_limit_white: Optional[float] = None, time_limit_black: Optional[float] = None) -> TestPackage:
        ...
        def start_batch():
            self._run_batch_worker(test_id, white_engine, black_engine, games, swap_colors, max_moves, start_fen, time_limit_white, time_limit_black)
        
        with self.lock:
            self._persist_test(batch)
            # Check if there's already a test running
            if self._running_test_id is not None:
                # Add to queue
                batch.status = "queued"
                self._persist_test(batch)
                self._test_queue.append((test_id, start_batch))
            else:
                # Start immediately
                self._running_test_id = test_id
    ...
    def _run_time_scaled_worker(self, test_id: str, white_engine: AILevel, black_engine: AILevel, time_limits: List[float], games_per_limit: int, swap_colors: bool, max_moves: int, start_fen: Optional[str]):
        ...
        # Collect results by time_limit for later aggregation
        results_by_tl: Dict[float, Dict[str, Any]] = {
            tl: {"white": 0, "black": 0, "draw": 0, "total_moves": 0, "games": 0}
            for tl in time_limits
        }

        def schedule_args(tl: float, game_idx: int):
            swap = swap_colors and (game_idx % 2 == 1)
            w_engine = black_engine if swap else white_engine
            b_engine = white_engine if swap else black_engine
            return {
        ...
        # Submit all tasks at once (all time limits, all games)
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = {}
            for tl in time_limits:
                for idx in range(games_per_limit):
                    params = schedule_args(tl, idx)
                    future = executor.submit(
                        _run_match_process,
                        params["w_engine"],
                        params["b_engine"],
            ...
            for future in as_completed(futures):
                params = futures[future]
                swap = params["swap"]
                tl = params["time_limit"]
                try:
                    winner, result_reason, move_history, _, _ = future.result()
                    moves = len(move_history)
                    ...
                    with self.lock:
                        ...
                        # Update time-limit-specific results
                        results_by_tl[tl]["games"] += 1
                        results_by_tl[tl]["total_moves"] += moves
                        if winner is None:
                            results_by_tl[tl]["draw"] += 1
                        else:
                            if (winner == PlayerColor.WHITE and not swap) or (winner == PlayerColor.BLACK and swap):
                                results_by_tl[tl]["white"] += 1
                            else:
                                results_by_tl[tl]["black"] += 1
                        
                        self._persist_test(test)
                except Exception as exc:
                    print(f"Time benchmark worker error: {exc}")

        # Aggregate results by time limit
        rows = []
        for tl in time_limits:
            r = results_by_tl[tl]
            games = r["games"] if r["games"] > 0 else 1
            rows.append(
                {
                    "time_limit": tl,
                    "results": {"white": r["white"], "black": r["black"], "draw": r["draw"]},
                    "games": games,
                    "avg_moves": r["total_moves"] / games,
                    "white_win_rate": r["white"] / games,
                    "black_win_rate": r["black"] / games,
                    "draw_rate": r["draw"] / games,
                }
            )
    ...
    def run_time_benchmark(self, white_engine: AILevel, black_engine: AILevel, time_limits: List[float], games_per_limit: int, swap_colors: bool, max_moves: int, start_fen: Optional[str] = None) -> TestPackage:
        ...
        def start_benchmark():
            self._run_time_scaled_worker(test_id, white_engine, black_engine, time_limits, games_per_limit, swap_colors, max_moves, start_fen)
        ...
        with self.lock:
            ...
            if self._running_test_id is not None:
                ...
                test.status = "queued"
                self._persist_test(test)
                self._test_queue.append((test_id, start_benchmark))
            else:
                # Start immediately
                self._running_test_id = test_id
                test.status = "running"
                self._persist_test(test)
                threading.Thread(target=start_benchmark, daemon=True).start()
