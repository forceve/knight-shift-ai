...
| **Ultimate** | `pesto_eval()` | **PeSTO评估**：阶段自适应评估（中局/残局混合） |
...
**重要说明**：评估函数现在只评估正常局面，不处理特殊局面（将死、和棋等）。特殊局面由搜索层通过 `evaluate_terminal_position()` 处理。这遵循 PeSTO 设计原则：保持评估函数"安静"（quiet）。
...
### 4. UltimateEngine.evaluate() - PeSTO 评估

**使用引擎**：Ultimate

**评估方式**：使用 PeSTO (Piece-Square Tables Only) 阶段自适应评估

**评估组件**：
```python
1. 阶段自适应评估 (Tapered Evaluation)
   - 根据棋盘上的棋子数量计算相位（phase）
   - 混合中局（MG）和残局（EG）评估
   - 公式：(mg_score * phase + eg_score * (MAX_PHASE - phase)) // MAX_PHASE

2. 子力值（中局/残局不同）
   - 中局：兵=82, 马=337, 象=365, 车=477, 后=1025
   - 残局：兵=94, 马=281, 象=297, 车=512, 后=936

3. 位置评估表（每个棋子类型都有中局/残局两套PST）
   - MG_PAWN, EG_PAWN
   - MG_KNIGHT, EG_KNIGHT
   - MG_BISHOP, EG_BISHOP
   - MG_ROOK, EG_ROOK
...
**特点**：
- 使用经过验证的标准评估表（来自 ChessProgramming Wiki）
- 阶段自适应，能更好适应中局到残局的过渡
- 架构与 Level3 相同，但使用 PeSTO 评估
- 评估函数保持"安静"，不处理特殊局面（由搜索层处理）

**代码位置**：`backend/app/engine/ultimate_engine.py`，使用 `pesto_eval.py` 中的 `pesto_eval()` 函数
...
│
UltimateEngine.evaluate() (重写，使用 PeSTO)
│
...
1. **搜索层**：负责处理特殊局面（将死、和棋、无子可动等）
   - 使用 `evaluate_terminal_position()` 函数
   - 在调用评估函数之前检查特殊局面
   - 如果发现特殊局面，直接返回评估值，不调用评估函数
...
4. **Ultimate**：使用 PeSTO 阶段自适应评估，架构同 Level3
5. **MCTS**：通过随机模拟评估
