...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{pairing_avg_score.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{consumption_Image.png}
...
Level 2 & 2.4 & 45 & 42.8 \\
Level 3 & 3.1 & 35 & 43.7 \\
Ultimate & 3.6 & 33 & 45.8 \\
\hline
\end{tabular}%
}
\end{table}

Human evaluation results in Table~\ref{tab:human_results} show progressive increase in perceived difficulty and corresponding decrease in human win rates. 
The difficulty ratings correlate strongly with self-play performance rankings, validating the level design. Qualitative feedback indicated that lower levels often made tactical blunders, 
while higher levels demonstrated better positional understanding and strategic planning.

\section{Performance Analysis}
This section interprets the experimental outcomes and explains what the results imply about
(i) the effective strength ordering of the engines,
(ii) the compute--strength trade-off under per-move time constraints, and
(iii) the practical impact of time budgeting on playing behavior.
To avoid redundancy with Section~\ref{sec:experimental_results}, we focus on trends and explanations rather than restating all raw counts.

\subsection{Overall Strength Ordering from M2M Tournaments}
The round-robin tournament results indicate a consistent stratification of playing strength across levels.
Across most pairings, stronger engines achieve higher average scores and remain robust after color swapping, suggesting that the observed ordering is not driven by first-move advantage alone.
In addition, higher levels tend to convert advantages more decisively, which is reflected by shorter average game lengths and fewer drawn outcomes in mismatched pairings.
These findings align with the intended difficulty scaling design: increasing search depth, adding pruning and move ordering, and enriching evaluation features collectively improve decision quality.

\subsection{Time-Scaled Benchmark (TSB): Compute--Strength Trade-off}
While fixed-time M2M tournaments provide a single operating point, real deployments often run under diverse latency constraints (e.g., mobile devices, congested servers, or interactive web sessions).
Therefore, we conduct a Time-Scaled Benchmark (TSB) to measure how each engine pairing behaves as the per-move time budget changes.

\subsubsection{Protocol and Metrics}
For each engine pairing, we sweep the per-move time limit over a range
$t \in \{0.1, 0.3, 0.5, \dots, 2.5\}$ seconds.
At each time limit, the two engines play $N$ games with colors reversed.
We report two primary metrics:
(1) \emph{score percentage} (win=1, draw=0.5, loss=0) and
(2) \emph{draw rate}.
This produces a time--performance curve that reveals both the marginal benefit of additional computation and the stability of the matchup.

\subsubsection{Interpreting TSB Curves}
TSB curves are informative in three ways.
First, the slope of the score curve reflects \emph{time sensitivity}: engines with iterative deepening or more effective pruning typically gain more from additional budget.
Second, the presence of a crossover point (if any) indicates a regime where algorithmic overhead dominates at low budgets, but deeper search becomes advantageous once sufficient time is available.
Third, the draw-rate curve helps distinguish whether extra computation yields (i) more decisive wins/losses or (ii) more accurate defense leading to additional draws.
Together, these patterns explain why some matchups look similar at a single time control but diverge under different budgets.

\subsubsection{Pairwise TSB Results}
Figures~\ref{fig:tsb_L1_L2}--\ref{fig:tsb_L3_Ult} visualize the time--score and time--draw relationships for each pairing.
For each plot, we highlight the dominant trend and the implied compute efficiency.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{time_scaled_l2_vs_l1_0db992b7-d377-4fab-ab8e-52fe8f197b73.png}
\caption{TSB curve for L1 vs L2. The plot reports score\% (win=1, draw=0.5, loss=0) and draw rate as functions of per-move time limit.}
\label{fig:tsb_L1_L2}
\end{figure}

\paragraph{L1 vs L2.}
As shown in Figure~\ref{fig:tsb_L1_L2}, L2 maintains a 100\% win rate across all time budgets, with the curve forming an almost horizontal line, indicating that its advantages in search depth and evaluation capability are sufficient to completely dominate L1.
Time extension does not yield additional benefits---this suggests that L1 makes too many errors, such that L2 can easily win even with minimal node computation.
The average number of moves fluctuates slightly between 60--80 moves, but shows no clear trend, indicating that game length is primarily determined by L1's defensive quality rather than time limits.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{time_scaled_l3_vs_l1_l3_vs_l1_tsb-a97f29ae-25a7-4ce4-9467-eb2cbe712a62 - 副本.png}
\caption{TSB curve for L1 vs L3.}
\label{fig:tsb_L1_L3}
\end{figure}

\paragraph{L1 vs L3.}
Figure~\ref{fig:tsb_L1_L3} shows that at very short time limits ($t<0.3$s), L3's win rate slightly decreases with some draws occurring, but when $t>0.5$s, the win rate rapidly increases and stabilizes at 100\%.
This indicates that L3 requires a certain amount of time to complete iterative deepening initialization or pruning ordering; once the time budget is sufficient, the improvement in tactical depth immediately becomes apparent.
The draw curve exhibits an isolated peak in the short-time interval, meaning that when computational resources are insufficient, L3 tends to enter repeated positions with L1.
The average game length slightly decreases as time increases, confirming that stronger engines are more likely to terminate games early under longer thinking time.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{time_scaled_ultimate_vs_l1_ultimate_vs_l1_tsb-d8ffe26e-d509-4926-ac7d-086e6b0aa2d8 - 副本.png}
\caption{TSB curve for L1 vs Ultimate.}
\label{fig:tsb_L1_Ult}
\end{figure}

\paragraph{L1 vs Ultimate.}
As shown in Figure~\ref{fig:tsb_L1_Ult}, Ultimate maintains a near 100\% win rate across the entire range, with only slight fluctuations when $t<0.3$s, after which it stabilizes.
This demonstrates that the PeSTO evaluation and deeper search depth completely overwhelm L1, with time limits having almost no effect on the outcome.
The draw percentage shows a slight peak in the initial phase ($\sim10\%$), possibly originating from repeated positions caused by opening symmetry.
The average number of moves fluctuates slightly between 60--100 moves, with an overall trend similar to L3 vs L1: stronger engines can terminate games faster under longer budgets.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{time_scaled_l3_vs_l2_5c8f6f07-0d65-4e1f-92c3-9c10b91d7a47 - 副本.png}
\caption{TSB curve for L2 vs L3.}
\label{fig:tsb_L2_L3}
\end{figure}

\paragraph{L2 vs L3.}
Figure~\ref{fig:tsb_L2_L3} shows that at shorter time limits ($t<0.5$s), the two engines are evenly matched, with L3's win rate fluctuating in the 50--70\% range, while L2 still occasionally wins or draws.
As time increases to 1.5--2.0s, L3's win rate gradually rises and approaches 100\%, while the draw percentage decreases, indicating that pruning and iterative deepening significantly improve tactical stability when time is sufficient.
The average number of moves is in the 150--200 move range and decreases as time increases, indicating faster position resolution and more stable endgame play.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{time_scaled_ultimate_vs_l2_ultimate_vs_l2_tsb-8d10b062-4b27-4cfa-9ddd-d5ad968ea056.png}
\caption{TSB curve for L2 vs Ultimate.}
\label{fig:tsb_L2_Ult}
\end{figure}

\paragraph{L2 vs Ultimate.}
As shown in Figure~\ref{fig:tsb_L2_Ult}, at short time limits ($t<0.4$s), Ultimate's win rate is approximately 85\% and slightly increases to 90--95\% over time, with L2 occasionally winning 1--3\% of games.
The draw rate shows a significant peak ($\sim50\%$) around $t\approx0.2$--0.3s, then stabilizes.
This indicates that at low time limits, both sides have not fully developed their strategies; as time increases, Ultimate's deep search advantage becomes apparent.
The average number of moves trend is consistent with other high-intensity matchups: longer time leads to shorter games, reflecting improved decision quality.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{time_scaled_ultimate_vs_l3_c05f7714-8afe-4554-833f-d7ac1077a06d - 副本.png}
\caption{TSB curve for L3 vs Ultimate.}
\label{fig:tsb_L3_Ult}
\end{figure}

\paragraph{L3 vs Ultimate.}
This is the most informative pairing.
As shown in Figure~\ref{fig:tsb_L3_Ult}, the win rate curves for both engines fluctuate dramatically at low time limits ($t<1$s), indicating that they are evenly matched with alternating wins at very short time limits.
When $t>2$s, L3's win rate once exceeds Ultimate's, then tends to equalize---this may reflect that the PeSTO evaluation provides limited benefits at high time budgets, or even causes decision bias due to over-evaluation in certain positions.
The draw rate oscillates at a high level in the 60--80\% range, indicating that the two engines have similar styles and find it difficult to convert advantages.
The average number of moves remains relatively stable between 80--110 moves, consistent with the characteristic that high-level games are longer and more stable.

\subsection{Practical Implications for Deployment}
The TSB results justify using a moderate default time limit that balances responsiveness and playing strength.
When deploying the system in a web-based interactive setting, overly large budgets can increase latency without proportional strength improvement, 
whereas overly small budgets may prevent deeper levels from expressing their intended advantage.
Therefore, we select the default time control to operate near the ``knee'' of the TSB curves, where additional computation begins to show diminishing returns while the relative ordering between levels remains stable.
...
Although the results show a clear strength ladder across L1--Ultimate, several limitations and practical constraints affect both absolute playing strength and the interpretation of the benchmark.

\textbf{Engine knowledge gaps.}
The engines do not use an opening book or endgame tablebases. As a result, opening play can be repetitive and vulnerable to prepared lines, 
and simplified endgames may be mishandled because the evaluator remains heuristic rather than tablebase-correct. In addition, 
the current evaluation does not explicitly encode some higher-order strategic concepts (e.g., long-term compensation, fortress detection), 
which can increase draw rates and slow conversion in advantage positions.

\textbf{Search and time-control constraints.}
Higher levels rely on alpha--beta with iterative deepening and quiescence search, but the implementation omits several widely used pruning and reduction techniques 
(e.g., null-move pruning, late-move reductions, aspiration windows). Consequently, the maximum reachable depth under a fixed per-move budget is limited. 
Moreover, 
computation time is not uniform across positions: tactically sharp positions can trigger deeper quiescence expansions and cause $2$--$3\times$ latency spikes relative to the average, 
leading to inconsistent responsiveness under tight time limits.

\textbf{Experimental scope and external validity.}
All experiments were executed on a single centralized host under fixed per-move time limits, and games were capped at a maximum ply limit to prevent excessively long matches. While these constraints ensure reproducibility, they can bias outcomes in slow endgames and do not capture all real-world time-control regimes. Human evaluation further involved a limited number of participants and subjective ratings, which may introduce sampling bias and variance in perceived difficulty.

\section{Future Work}
Future work will focus on improving (i) playing strength per unit compute, (ii) stability and calibration of difficulty levels, and (iii) benchmarking rigor.

\textbf{Stronger play under the same time budget.}
A natural next step is to incorporate an opening book and lightweight endgame support (e.g., selected tablebases or endgame-specific heuristics) to reduce early-game predictability and improve conversion in simplified positions. On the search side, adding established enhancements such as aspiration windows, null-move pruning, late-move reductions, and more principled move ordering (e.g., history heuristics, killer moves) can substantially increase effective depth without increasing the time limit. Transposition tables can be further improved via larger capacities, better replacement policies, and storing richer node information.

\textbf{Learning-based evaluation and systematic tuning.}
While this project prioritizes interpretable heuristics, the framework can be extended with data-driven tuning (e.g., self-play parameter optimization / Texel-style tuning) to replace manual weight selection. Another promising direction is integrating efficient neural evaluators (e.g., NNUE-style inference or quantized CNNs) as drop-in evaluators, enabling stronger positional judgment while keeping inference cost practical on CPU.

\textbf{Difficulty calibration and user experience.}
Beyond fixed levels, we plan to implement dynamic difficulty adjustment that adapts depth, randomness, and risk preferences based on a player’s recent performance. Additionally, calibrating each level to a target Elo-like scale (rather than only relative ordering) would make difficulty settings more interpretable to users and more comparable across machines and time controls.

\textbf{More rigorous evaluation.}
Finally, future experiments will expand the number of games and use stronger statistical testing (e.g., confidence intervals or SPRT-style evaluation) to quantify uncertainty. We also plan to evaluate with multiple opening positions (opening randomization) and to report time-scaled performance curves more systematically, improving external validity and reducing bias from repetitive openings.

\textbf{Human-like play and style control.}
Beyond raw strength, an important direction is to make the engine behave more like a human opponent. 
This can be achieved by introducing controlled imperfection and stylistic diversity, such as (i) stochastic move selection (e.g., sampling from the top-$k$ moves using a softmax over their search scores), 
(ii) calibrated blunder/noise models that depend on tactical complexity and time pressure, 
(iii) human-like time management (spending more time in sharp positions and less in forced lines), and 
(iv) opening variety to reduce repetitive play. 
In addition, we plan to incorporate explainable move annotations (e.g., ``improves king safety'' / ``wins a piece''), improving perceived intelligence and learnability for human players.

\textbf{Elo-based difficulty system.}
Instead of defining difficulty purely by depth or heuristic complexity, future versions will provide an Elo-based difficulty scale that maps a user-selected target rating to engine parameters (time per move, depth limits, randomness, pruning aggressiveness, and evaluation weights). 
We will estimate ratings by running large-scale self-play and cross-play tournaments among parameter configurations, optionally including one or more anchor opponents (e.g., a fixed external engine at known settings) to stabilize the scale. 
The resulting calibration curve enables a consistent ``Difficulty $\rightarrow$ expected win rate'' interpretation across machines and time controls, and supports personalized matchmaking by tracking a player's estimated rating and selecting an engine configuration with a desired expected score.


\section{Conclusion}
This paper presented a web-based multi-level chess AI system that exposes graduated playing strength through systematic control of search horizon, 
pruning efficiency, evaluation complexity, and move-selection strategy. Across four primary levels (L1--L3 + Ultimate), 
the engines progress from greedy one-ply selection to alpha--beta minimax with iterative deepening and quiescence search, 
with the Ultimate level employing PeSTO-style tapered evaluation for phase-adaptive position assessment. 

Experimental results from self-play tournaments and human evaluation demonstrate clear separation between difficulty tiers and a consistent cost--strength trade-off: 
stronger levels achieve higher win rates at the expense of greater per-move computation. Overall, the project validates that classical search-based methods remain effective for building practical, 
resource-aware, adjustable chess opponents, and it provides a modular foundation for future improvements in search efficiency, learned evaluation, and difficulty calibration.
