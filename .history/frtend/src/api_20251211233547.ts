...
function getApiBase(): string {
  ...
  if (typeof window !== "undefined") {
    ...
    // If accessed via frp-gym.com, use frp-gym.com:57367 for backend
    if (hostname === "frp-gym.com" || hostname.includes("frp-gym.com")) {
      return "http://frp-gym.com:57367";
    }
    
    // If accessed via frp-lab.com, use frp-lab.com with same port pattern
    if (hostname === "frp-lab.com" || hostname.includes("frp-lab.com")) {
      // Assuming similar port pattern, adjust if needed
      return `http://frp-lab.com:${port}`;
    }
