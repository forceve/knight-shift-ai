...
The chess board dynamically resizes using \texttt{Math.min(520, window.innerWidth - 48)}, while grid layouts transition from single-column to multi-column arrangements (e.g., \texttt{grid-cols-1 lg:grid-cols-3}) and flex containers adapt orientation (\texttt{flex-col lg:flex-row}) to maximize screen utilization.
