...
\begin{itemize}
    ...
    using proven evaluation tables from chess programming literature. Time limit of 1.2 seconds per move.
\end{itemize}
...
Our approach combines classical game-tree search algorithms and Monte Carlo Tree Search with theory-driven evaluation functions, implementing three primary difficulty levels (Level 1 through Level 3) plus an Ultimate level that scale from simple greedy evaluation to sophisticated minimax with iterative deepening and quiescence search, and finally to MCTS with optional CNN-based evaluation. Experimental results demonstrate clear performance stratification, with each level offering appropriate challenges for different skill levels while maintaining reasonable computational efficiency.
