...
Alpha--beta pruning was introduced to help with this problem by pruning branches that cannot affect the final minimax value, making it one step forward to \emph{type B} algorithms.
...
Historically, alpha--beta pruningwas discovered independently several times in the late 1950s and early 1960s and later analyzed in detail by Knuth and Moore in their work 
...
MCTS employs a breadth-first exploration strategy based on random selection and iteratively builds the search tree, 
making it especially good at hadling large branching factors, as in Go.
The success of AlphaGo first showed the power of MCTS in board games, 
demonstrating the effectiveness of MCTS for solving complex board games such as Go and its potential combined with deep neural networks~\cite{silver2016}.
...
Historically, alpha--beta was discovered independently several times in the late 1950s and early 1960s and later analyzed in detail by Knuth and Moore in their work "An Analysis of Alpha-Beta Pruning"~\cite{knuth1975},
in which they showed that effective branching factor can be reduced roughly to the square root of that of plain minimax. 
In practice, alpha--beta pruning, combined with appropriate move-ordering heuristics, allows significantly deeper search within the same budget.
