...
This is the most sensitive test of evaluation quality under comparable search architecture.
As shown in Figure~\ref{fig:tsb_L3_Ult}, a small but consistent advantage indicates that improved evaluation yields better move ordering and leaf scoring, especially when the search reaches tactically stable positions more frequently at higher budgets.
If the curves remain close across all time limits, it suggests that search dominates performance in this configuration and the evaluation upgrade provides only marginal gains under the tested settings.
...
When deploying the system in a web-based interactive setting, overly large budgets can increase latency without proportional strength improvement, whereas overly small budgets may prevent deeper levels from expressing their intended advantage.
