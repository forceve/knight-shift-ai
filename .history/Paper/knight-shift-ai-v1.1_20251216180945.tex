...
(i) search depth and time-bounded search behavior, (ii) pruning and move ordering strategies, (iii) evaluation function complexity, and (iv) move selection stochasticity.
...
\max_{a\in A(s)} \text{minimax}(T(s,a),d-1), & \text{max node} \\
\min_{a\in A(s)} \text{minimax}(T(s,a),d-1), & \text{min node}.
\end{cases}
\]
...
From advanced levels onward, we apply alpha--beta pruning to reduce the effective branching factor by eliminating branches that cannot affect the final minimax decision.
...
\begin{enumerate}
    ...
    \item \textbf{Move selection stochasticity:} controlled randomness at weaker levels to reduce strength and introduce variety.
\end{enumerate}
...
L2  & Minimax (d=2)  & No & No & No & Material-only; random tie-break \\
L3  & Minimax        & Yes & Yes (to d=4) & Yes & Rich positional eval; deterministic \\
Ult.& Minimax        & Yes & Yes (to d=5) & Yes & PeSTO tapered eval; deterministic \\
\hline
\end{tabular}
\end{table}
...
implementing four primary difficulty levels (L1--L3 + Ultimate) for the main experiments that scale from simple greedy evaluation to sophisticated minimax with iterative deepening and quiescence search, 
