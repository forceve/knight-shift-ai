...
showing that even limited computing power could produce competitive play on the lousy hardware of the 1950s and 1960s.
...
In our tree search context like chess, minimax assumes optimal play from both sides, 
and recursively chooses the leaf nodes back to the root to choose moves that maximize the outcome for it's side. 

Pruning first came in XXXX ....... , and take a step forward to \emph{type B} algorithms.
Alpha--Beta pruning is a technique to prune the branches of the tree that are guaranteed to be worse than the best move found so far.
It is a simple and effective technique to reduce the number of nodes searched.
It is first introduced in 1975 by Arthur Samuel in his work "Some Studies in Machine Learning Using the Game of Checkers".
It is a simple and effective technique to reduce the number of nodes searched.

The Monte Carlo Tree Search (MCTS).

As for iterative deepening, XXXXXX

One of the earliest "True \emph{type B}" algorithms is XXXXXX

For my system, XXXXXXX.

\subsection{Search Techniques}
Building on the tree-search view of chess, a long line of work has developed concrete search techniques with increasing depth, selectivity, and efficiency.

The simplest approach is a one-ply or greedy strategy that evaluates only the immediate successor positions and chooses the move with the best heuristic score. 
Early game-playing programs such as Samuel's checkers system~\cite{samuel1959} combined shallow lookahead with hand-crafted evaluation functions and simple learning rules, 
demonstrating that even limited search could produce competitive play on the hardware of the 1950s and 1960s.

A more principled alternative is minimax search, rooted in game-theoretic work by von Neumann and others on zero-sum games and the minimax theorem~\cite{vonNeumann1928,vonNeumann1944}. 
In a game-tree context, minimax assumes optimal play from both sides and recursively propagates values from leaf nodes back to the root to choose moves that maximize the eventual outcome from the agent's perspective. 
However, the time complexity of full-width minimax is exponential in search depth, making straightforward implementations impractical for deep search in chess.

Alpha--beta pruning was introduced to alleviate this problem by pruning branches that cannot affect the final minimax value. 
Historically, alpha--beta was discovered independently several times in the late 1950s and early 1960s and later analyzed in detail by Knuth and Moore~\cite{knuth1975}, 
who showed that with good move ordering the effective branching factor can be reduced roughly to the square root of that of plain minimax. 
In practice, alpha--beta pruning, combined with appropriate move-ordering heuristics, allows significantly deeper search within the same time budget.

Iterative deepening depth-first search further improves robustness and time management by repeatedly searching the tree to increasing depths. 
Although it appears wasteful at first glance, iterative deepening has been shown to be beneficial for move ordering and overall performance in chess programs~\cite{marsland1991,korf1985}. 
Information from shallower iterations (such as the current best move or principal variation) is used to order moves in deeper searches, which in turn increases the effectiveness of alpha--beta pruning and provides natural anytime behavior under strict time limits.

More recently, Monte Carlo Tree Search (MCTS) has emerged as an alternative search paradigm, especially in games such as Go. 
Frameworks such as Coulom's Monte-Carlo tree search with selective backup~\cite{coulom2006} and the UCT algorithm of Kocsis and Szepesv\'ari~\cite{kocsis2006} combine tree search with stochastic rollouts and bandit-based exploration, 
trading exhaustive lookahead for sampled trajectories guided by statistical estimates. 
These methods have been highly successful in domains where accurate heuristic evaluation is difficult to design by hand.

In our system, we adopt the classical minimax framework with alpha--beta pruning and iterative deepening as the core search technique, 
since it remains highly effective for chess when paired with strong evaluation functions and allows us to control difficulty through search depth, pruning settings, and time limits. 
We do not employ MCTS in this work, but discuss it as a contrasting search paradigm that emphasizes sampling over deterministic deep search.
