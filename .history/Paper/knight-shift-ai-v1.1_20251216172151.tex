...
The Ultimate level uses the same search architecture as Level 3 but employs PeSTO (Piece-Square Tables Only) tapered evaluation, and more advanced quiescence search,
...
and iterative deepening to provide natural time management and robustness under computational constraints (in Level 3 and Ultimate, as well as in experimental Level 4). 
...
ensuring that evaluation occurs at relatively stable positions (implemented in Level 3 and Ultimate, as well as in experimental Level 4). 
...
Additionally, experimental variants (L4/L5) were implemented with enhanced evaluation functions and different search configurations. Level 4 extends Level 3 with phase-adaptive evaluation and iterative deepening up to depth 6, 
while Level 5 implements fixed-depth alpha-beta at depth 3. These engines were not included in the primary benchmark, but are available for extended testing.
...
The web interface provides an intuitive chess experience with drag-and-drop move input, real-time board updates, and comprehensive game controls. 
The interface includes a move history panel, status indicators for check and game end conditions, and support for game resignation. 
The replay viewer allows us to step through completed games with by-step, pause, play, and multiple speed options, supporting both H2M and M2M matches.
