...
\begin{table}[t]
\centering
\caption{M2M Round-Robin Results (Score\%)}
\label{tab:m2m_score}
\scriptsize
...
\toprule
\textbf{Engine} & \textbf{vs L1} & \textbf{vs L2} & \textbf{vs L3} & \textbf{vs Ult.} & \textbf{Avg.} \\
\midrule
L1   & --  & xx.x & xx.x & xx.x & xx.x \\
L2   & xx.x & --  & xx.x & xx.x & xx.x \\
L3   & xx.x & xx.x & --  & xx.x & xx.x \\
Ult. & xx.x & xx.x & xx.x & --  & xx.x \\
\bottomrule
\end{tabular}
\end{table}

\noindent Table~\ref{tab:m2m_score} reports the overall score percentage
