...
\\begin{equation}
    \label{eq:minimax_base}
    \text{minimax}(s,d)=
    \begin{cases}
    V(s), & d=0 \ \text{or}\ s\ \text{is terminal},\\
    \text{mm}(s,d), & \text{otherwise}.
    \end{cases}
    \end{equation}
    
    \begin{equation}
    \label{eq:minimax_rec}
    \text{mm}(s,d)=
    \begin{cases}
    \max\limits_{a\in A(s)} \text{minimax}(T(s,a),d-1), & s\ \text{is a max node},\\
    \min\limits_{a\in A(s)} \text{minimax}(T(s,a),d-1), & s\ \text{is a min node}.
    \end{cases}
    \end{equation}
