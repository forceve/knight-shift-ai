...
For the lower difficulty levels (Level 1 and Level 2), we use simple greedy evaluation or minimax search without pruning, 
which provides conceptual clarity and computational efficiency. 
For intermediate levels (Level 3, Level 4, and Level 5), we employ alpha--beta pruning with iterative deepening, 
which provides fine-grained control over search depth and pruning behavior. 
The Ultimate level uses the most sophisticated alpha--beta search with iterative deepening and quiescence search. 
Additionally, the system includes separate MCTS-based engines (not part of the main difficulty progression) that employ Monte Carlo Tree Search guided by CNN-based evaluation, 
...
In the MCTS-based engines (separate from the main difficulty progression), this CNN provides position scores or policy hints that guide Monte Carlo Tree Search under a fixed computation budget, while the main difficulty levels (Level 1 through Ultimate) rely more heavily on theory-driven heuristic evaluation. 
...
We use a modular design with separate components for move generation, tree search, and evaluation, and employ iterative deepening with time limits in Level 3, Level 4, and Ultimate difficulty levels. 
Although we do not implement advanced features such as transposition tables, opening books, or endgame tablebases, 
the core architectural principles---modularity and time-aware iterative deepening---guide our design for operating under limited hardware and exposing controllable difficulty levels.
...
Instead of targeting specific Elo ratings, we define five discrete difficulty levels (Level 1 through Ultimate) by controlling search depth, pruning aggressiveness, evaluation complexity, 
and move selection randomness. Additionally, the system includes separate MCTS-based engines with CNN-based evaluation for research purposes. 
...
and iterative deepening to provide natural time management and robustness under computational constraints (in Level 3, Level 4, and Ultimate). 
...
(1) search depth variation (from depth 1 at Level 1 to iterative deepening up to depth 6 at Level 4, and depth 5 at Ultimate), 
...
\begin{itemize}
    \item \textbf{Level 1 (Greedy)}: Evaluates all legal moves one ply deep, selecting the best immediate position based on material and positional heuristics. No search tree, no pruning.
    \item \textbf{Level 2}: Implements minimax search at fixed depth 2 without alpha-beta pruning. Uses material-only evaluation and random tie-breaking among top 3 moves to keep play weak.
    \item \textbf{Level 3}: Implements alpha-beta pruning with iterative deepening up to depth 4, quiescence search, and move ordering. Time limit of 0.6 seconds per move.
    \item \textbf{Level 4}: Extends Level 3 with enhanced evaluation (phase-adaptive features, pawn structure, king safety) and iterative deepening up to depth 6. Time limit of 1.5 seconds per move.
    \item \textbf{Level 5}: Implements alpha-beta pruning at fixed depth 3 with move ordering, but without iterative deepening or quiescence search.
    \item \textbf{Ultimate}: Implements alpha-beta pruning with iterative deepening up to depth 5, quiescence search, and the most sophisticated evaluation function. Time limit of 1.2 seconds per move.
\end{itemize}
...
\hline
\textbf{White vs Black} & \textbf{Level 1} & \textbf{Level 2} & \textbf{Level 3} & \textbf{Level 4} & \textbf{Ultimate} \\
\hline
Level 1 & - & 15 & 5 & 2 & 0 \\
Level 2 & 85 & - & 25 & 8 & 1 \\
Level 3 & 95 & 75 & - & 35 & 5 \\
