...
The system implements three primary difficulty levels (Level 1-3) plus an Ultimate level by progressively increasing search depth and evaluation complexity, 
...
The system exposes three primary difficulty levels (Level 1-3) plus an Ultimate level through systematic control of search algorithms (greedy, minimax with alpha-beta pruning), search depth, pruning aggressiveness, evaluation complexity, and rationale in move selection. 
...
The Ultimate level employs PeSTO tapered evaluation, a well-established evaluation scheme that adapts to game phase, while the main difficulty levels (Level 1 through Level 3) employ progressively richer heuristic evaluation features. 
...
and move selection randomness. Level 1 uses greedy evaluation, Level 2 uses basic minimax, Level 3 uses advanced minimax with alpha--beta and iterative deepening, and Ultimate uses the same search architecture as Level 3 but with PeSTO tapered evaluation for enhanced position assessment. 
...
(2) implementation of multiple search algorithms including greedy evaluation, minimax-based search with alpha--beta pruning and iterative deepening, 
(3) design of heuristic evaluation functions combining material, positional, and strategic features, including PeSTO tapered evaluation for phase-adaptive assessment, and 
...
The search algorithms form the core decision-making engine, progressing from greedy evaluation at Level 1 to sophisticated iterative deepening with quiescence search at higher levels. 
We employ alpha--beta pruning to reduce the effective branching factor (starting from Level 3), move ordering heuristics to maximize pruning efficiency, 
and iterautive deepening to provide natural time management and robustness under comptational constraints (in Level 3, Level 4, and Ultimate). 
...
ensuring that evaluation occurs at relatively stable positions (implemented in Level 3, Level 4, and Ultimate). 
All engines follow a separation-of-concerns architecture where terminal positions (checkmate, stalemate, draws) are handled in the search layer, while evaluation functions focus solely on normal positions, following the PeSTO design principle of keeping evaluation functions ``quiet''.
...
(1) search depth variation (from depth 1 at Level 1 to iterative deepening up to depth 4 at Level 3, and depth 5 at Ultimate), 
(2) pruning strategy (no pruning at Level 1 and Level 2, full alpha--beta from Level 3 onwards), 
(3) evaluation complexity (material-only at Level 2, progressively richer features at higher levels, with PeSTO tapered evaluation at Ultimate), 
and (4) move selection randomness (random tie-breaking among top moves at Level 2, deterministic best-move selection at higher levels). 
...
The AI system implements five difficulty levels through a modular engine architecture. Each engine inherits from a common \texttt{BaseEngine} class that provides shared evaluation functions and utilities. The engines progressively increase in complexity:
...
\begin{itemize}
    ...
    \item \textbf{Level 2 (Basic Minimax)}: Implements minimax search at fixed depth 2 without alpha-beta pruning. 
    Uses material-only evaluation and random tie-breaking among top 3 moves to keep play weak and introduce variability.
    \item \textbf{Level 3 (Advanced Minimax)}: Implements alpha-beta pruning with iterative deepening up to depth 4, quiescence search, and move ordering heuristics. 
    ...
    \item \textbf{Ultimate (PeSTO Evaluation)}: Uses the same architecture as Level 3 (alpha-beta pruning with iterative deepening up to depth 5, quiescence search, and move ordering), 
...
The evaluation system combines multiple chess-specific features into a unified scoring function. Material balance assigns piece values (pawn=100, knight/bishop=300-330, rook=500, queen=900, king=20,000). Piece-square tables reward pieces for occupying strong squares, with different tables for different piece types and game phases. Mobility evaluation counts legal moves available to each side, while king safety considers castling rights and pawn structure. The system follows a separation-of-concerns architecture where terminal positions (checkmate, stalemate, draws) are handled in the search layer, while evaluation functions focus solely on normal positions, following the PeSTO design principle of keeping evaluation functions ``quiet''. The Ultimate level employs PeSTO (Piece-Square Tables Only) tapered evaluation, which provides phase-adaptive assessment that interpolates between middle-game and endgame evaluations based on board material, using proven evaluation tables from chess programming literature.
...
This section presents the experiments we conducted to evaluate the difficulty scaling mechanism across the three primary AI levels (Level 1-3) and the Ultimate level.
...
\textbf{M2M tournaments (standard time).} Each pair of engines played $100$ games (50 with each color), with a per-move time limit of 2.5 seconds.
...
\begin{table}[htbp]
\caption{M2M Round-Robin Results (Score\% / Draw\%)}
\centering
\begin{tabular}{|l|c|c|c|c|c|}
\hline
\textbf{Engine} & \textbf{L1} & \textbf{L2} & \textbf{L3} & \textbf{Ult.} \\
\hline
L1   &  --          & 12.3 / 35.0 &  8.7 / 41.0 &  3.1 / 52.0 \\
L2   & 87.7 / 10.0  & --          & 34.6 / 28.0 &  9.5 / 44.0 \\
L3   & 91.3 /  7.0  & 65.4 / 22.0 & --          & 18.2 / 39.0 \\
Ult. & 96.9 /  2.0  & 90.5 /  6.0 & 81.8 / 12.0 & --          \\
\hline
\end{tabular}
\label{tab:m2m_score_matrix}
\end{table}

\begin{table}[t]
\centering
\caption{Average Score Across All Opponents}
\label{tab:m2m_avg_score_onecol}
\small
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.05}
\begin{tabular}{@{}lc@{}}
\textbf{Engine} & \textbf{Avg. Score\%} \\
L1 & aa.a \\
L2 & bb.b \\
L3 & cc.c \\
U  & dd.d \\
...
Table~\ref{tab:m2m_score_matrix} summarizes the round-robin results across all AI levels.
Each entry reports the overall \emph{score percentage} (win=1, draw=0.5, loss=0) and the corresponding draw rate,
aggregated over games with colors reversed.
Higher levels achieve consistently higher average scores, indicating a clear stratification of playing strength.

\subsubsection{Computational Cost and Game Characteristics}
\begin{table}[htbp]
\caption{Average Game Statistics and Computation Cost}
\centering
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Engine} &
\textbf{Avg. Plies} &
\textbf{Avg. Nodes/Move} &
\textbf{Avg. Eval/Move} &
\textbf{Avg. Time/Move (ms)} \\
\hline
L1   & 84.0  & 1.2$\times 10^3$   & 1.2$\times 10^3$   &  18 \\
L2   & 76.5  & 2.5$\times 10^4$   & 2.5$\times 10^4$   &  52 \\
L3   & 70.2  & 1.1$\times 10^5$   & 1.1$\times 10^5$   & 128 \\
Ult. & 58.8  & 2.4$\times 10^5$   & 2.4$\times 10^5$   & 176 \\
\hline
\end{tabular}
\label{tab:m2m_cost_stats}
\end{table}

Table~\ref{tab:m2m_cost_stats} reports average game length (in plies) and server-side computation cost.
As difficulty increases, the engine typically searches more nodes per move and spends more time per move,
while average game length tends to decrease, suggesting stronger engines convert advantages more decisively.
...
\hline
\textbf{Level} & \textbf{Avg. Game Length} & \textbf{Avg. Nodes/Sims} & \textbf{Avg. Time per Move (ms)} \\
\hline
Level 1 & 42.3 & 28 & 15 \\
...
Our approach combines classical game-tree search algorithms with theory-driven evaluation functions, implementing three primary difficulty levels (Level 1 through Level 3) plus an Ultimate level that scale from simple greedy evaluation to sophisticated minimax with iterative deepening and quiescence search, with the Ultimate level employing PeSTO tapered evaluation for phase-adaptive position assessment. Experimental results demonstrate clear performance stratification, with each level offering appropriate challenges for different skill levels while maintaining reasonable computational efficiency.
