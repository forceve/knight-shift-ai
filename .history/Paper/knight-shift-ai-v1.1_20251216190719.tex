...
The frontend follows a component-based design pattern, with reusable UI components including \texttt{Controls}, \texttt{MoveList}, and \texttt{StatusBar} that encapsulate specific functionality. 
The application structure separates concerns into distinct layers: page components (\texttt{HomePage}, \texttt{PlayPage}, \texttt{M2MPage}, \texttt{ReplayPage}, \texttt{HistoryPage}) for routing and layout, shared components for common UI elements, and a centralized API layer (\texttt{api.ts}) for backend communication. 
This modular approach facilitates maintainability, code reuse, and independent testing of individual features.

\subsubsection{Responsive Layout Design}
The interface employs Tailwind CSS utility classes to achieve adaptive layouts that seamlessly adjust to different screen sizes. 
The chess board dynamically resizes based on viewport width using \texttt{Math.min(520, window.innerWidth - 48)} to ensure optimal display on mobile, tablet, and desktop devices. 
Grid layouts transition from single-column on mobile to multi-column arrangements on larger screens (e.g., \texttt{grid-cols-1 lg:grid-cols-3}), while flex containers adapt their orientation (\texttt{flex-col lg:flex-row}) to maximize screen real estate utilization.

\subsubsection{Page-Based Navigation}
The application implements a multi-page architecture using React Router, with dedicated pages for distinct functionalities: the home page provides an overview and quick access to features, the play page handles human vs. human and human vs. AI gameplay, the M2M page manages machine vs. machine testing and batch comparisons, the replay page enables step-by-step game analysis, and the history page displays completed match records. 
Each page maintains its own state management and UI layout, enabling focused user experiences for different use cases.

\subsubsection{Interactive Features}
The interface provides drag-and-drop move input with visual feedback, real-time board updates, and comprehensive game controls including mode selection, AI level configuration, and time limit settings. 
The move history panel displays moves in standard algebraic notation (SAN) format, while status indicators highlight check conditions and game end states. 
The replay viewer supports step-by-step navigation, pause, play, and multiple playback speeds, accommodating both H2M and M2M match analysis.
