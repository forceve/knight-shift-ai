...
Building on this tradition, modern chess engines combine powerful search algorithms with sophisticated evaluation functions, and continue to provide a platform for artificial intelligence studies.
...
\section{Introduction}
Chess was among the first board games in which computer programs rivaled and eventually surpassed top human players, making it a classic subject for research in artificial intelligence for decades. 


Early work such as Shannon's 1950 paper ``Programming a Computer for Playing Chess''~\cite{shannon1950} explicitly framed a chess-playing program as a prototype for more general ``thinking machines'': 
systems that could, in principle, design filters and relay circuits, optimize telephone routing, perform symbolic algebra and language translation, 
plan strategy in simplified war games, orchestrate music, or carry out logical reasoning. In this view, computer chess is not an end in itself, 
but a concrete sandbox for studying how machines can make complex, strategic decisions under uncertainty and resource constraints.


In this work, we design and evaluate a multi-level chess AI system that exposes four distinct difficulty levels under limited computational resources. 
The system combines classical tree-search algorithms with two families of evaluation functions: theory-driven positional heuristics inspired by common chess engines, 
and CNN-based learned evaluators of board patterns. Difficulty scaling is achieved by systematically varying search depth, pruning aggressiveness, and evaluation complexity, 
together with controlled randomness at lower levels. We then use extensive self-play and human--AI experiments, together with basic statistical analysis of win rates and game lengths, 
to quantify the performance gap between levels and to study the trade-off between playing strength and computational cost.
