...
\title{A Multi-Level Board-Game AI Based on Minimax and Heuristic Evaluation}

\author{
\IEEEauthorblockN{First Author, Second Author}
\IEEEauthorblockA{
Department of XXX, University of YYY\\
Email: first.author@xxx.edu, second.author@xxx.edu}
}

\maketitle

\begin{abstract}
This paper presents a multi-level chess AI system designed as a course project in an Artificial Intelligence Workshop. The system implements four distinct difficulty levels, ranging from a shallow minimax baseline to an advanced engine with iterative deepening and sophisticated evaluation functions. Our approach combines classical game-tree search algorithms, including Minimax with Alpha--Beta pruning, iterative deepening, and quiescence search, with heuristic evaluation functions that incorporate material balance, piece-square tables, mobility, pawn structure, and king safety. The difficulty scaling is achieved through systematic variations in search depth, pruning strategies, and evaluation complexity. Experimental results demonstrate clear performance separation between levels, with the strongest engine achieving significantly higher win rates against lower-level opponents while maintaining reasonable computational efficiency. The system supports multiple game modes including human-versus-machine and machine-versus-machine matches, enabling both interactive play and systematic performance evaluation. This work validates the effectiveness of classical search algorithms in resource-constrained environments and provides a foundation for educational applications and future enhancements, such as integrating machine learning-based evaluation functions.
\end{abstract}

\begin{IEEEkeywords}
Game-tree search, Minimax, Alpha--Beta pruning, Heuristic evaluation, Board-game AI, Difficulty scaling
\end{IEEEkeywords}
