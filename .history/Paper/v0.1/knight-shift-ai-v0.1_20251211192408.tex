...
Chess was among the first board games in which computer programs (agents) rivaled and eventually surpassed top human players, making it a classic subject for research in artificial intelligence for decades.
...
However, in many circunstances, we may want agents with \emph{adjustable} strength that can provide meaningful practice for human players and that can operate effectively under tight time and hardware constraints.
This paper will introduce the design of a multi-level chess AI system, and the evaluation of its performance on different difficulty levels, even under limited computational resources.

We address these challenges by designing a multi-level chess AI system built on classical tree search algorithms with two families of evaluation functions: 
...
We further conduct both self-play and human-AI experiments, with both yes and no limited searching time, 
and use basic statistical analysis to quantify the performance differences between levels and to study the relationship between playing strength and computational effort for different difficulty levels.
\section{Introduction}
Chess was among the first board games in which computer programs rivaled and eventually surpassed top human players, making it a classic subject of research in artificial intelligence for decades.

Early work such as Shannon's 1950 paper ``Programming a Computer for Playing Chess''~\cite{shannon1950} emphasized that chess agents are prototypes for more general ``thinking machines'': 
systems that could, in principle, perform complex strategic decisions like human beings. In this view, computer chess is not an end in itself, 
but a concrete sandbox for studying how machines can make complex, strategic decisions under uncertainty and resource constraints.
Building on this tradition, modern chess engines combine powerful search algorithms with sophisticated evaluation functions, and continue to provide a platform for studying the efficiency and limitations of classical AI methods.

Most contemporary chess engines and research efforts focus on maximizing playing strength, often assuming ample computational resources and targeting superhuman performance. 
However, in many educational and practical settings, the goal is different: we want agents with \emph{adjustable} strength that can provide meaningful practice for human players and that can operate effectively under tight time and hardware constraints.
Designing such a system raises several challenges, including how to expose multiple difficulty levels in a principled way, how to balance search depth and evaluation complexity, and how to quantify the trade-off between playing strength and computational cost.

In this work, we address these challenges by designing a multi-level chess AI system built on classical tree-search algorithms and two families of evaluation functions: 
theory-driven positional heuristics inspired by common chess engines, and CNN-based learned evaluators of board positions and local patterns.
The system exposes four difficulty levels through systematic control of search depth, pruning aggressiveness, evaluation complexity, and controlled randomness in move selection.
We further conduct both self-play and human--AI experiments, with and without strict time limits, and use basic statistical analysis of win rates and game lengths to quantify performance differences between levels and to study the relationship between playing strength and computational effort.

The main contributions of this paper are as follows:
\begin{itemize}
    \item We design and implement a multi-level chess AI system that offers four distinct difficulty levels and supports multiple game modes, including human-versus-machine, machine-versus-machine, limited-time play, and batch testing.
    \item We propose a hybrid evaluation framework that combines theory-driven positional heuristics with CNN-based learned evaluators, and integrate it with classical tree-search techniques for practical use under resource constraints.
    \item We provide an experimental and statistical study of the system's behavior across difficulty levels, highlighting the trade-off between playing strength and computational cost in resource-constrained settings.
\end{itemize}
