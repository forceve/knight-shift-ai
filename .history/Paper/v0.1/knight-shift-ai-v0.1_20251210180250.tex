...
% Choose a CJK font available in your system / Overleaf
% 在本地请把 SimSun 换成你有的中文字体，比如思源宋体等
...
\begin{abstract}
This section is a concise summary of the whole paper (200--250 words).

本节是全文的精简版，总结「问题背景 + 方法 + 实验 + 结论」四个要素。写作时可以按下面思路组织：

\begin{itemize}
    \item \textbf{Background / 背景（1--2 句）}：
    简短说明任务是什么、属于哪个领域、为什么有意义。\\
    Example: ``This paper presents a multi-level AI agent for the board game Gomoku, designed as a course project in an Artificial Intelligence Workshop.''
    
    \item \textbf{Method / 方法（2--3 句）}：
    用非常高层的语言说你用到了哪些关键技术，如 Minimax, Alpha--Beta, heuristic evaluation, difficulty scaling 等。中文可以注释思路：说明你是「基于什么算法 + 采用了什么设计策略」。
    
    \item \textbf{Results / 结果（2--3 句）}：
    用定量语言概括主要实验结果，例如：
    ``The strongest level outperforms the baseline random agent by XX\% win rate and demonstrates a clear strength gap compared to lower levels.''\\
    中文可以解释：点出胜率、搜索效率、棋力分级是否明显。
    
    \item \textbf{Conclusion / 意义（1--2 句）}：
    最后一句给出整体评价与潜在应用。比如：本项目验证了经典搜索算法在有限算力和教学场景中的有效性，并为后续引入学习型评估函数提供了基础。
\end{itemize}

写摘要时注意：不出现公式和引用，不细讲实现细节，用一般现在时，整体一段。
\end{abstract}
