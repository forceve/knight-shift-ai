...
Since Claude Shannon's seminal work on Computer Playing Chess~\cite{shannon1950}, board games like chess have been seen as a typical game-tree search problem, 
in which the agent expands and explores nodes in a tree of possible moves and counter-moves. 
However, from the very beginning, Shannon has already pointed out: Since chess is such a big game in which each move can have more than XX possible moves, 
making the full game tree is astronomically large, making exhaustive search simply infeasible. 
Instead, chess programs must perform depth-limited search and use heuristic evaluation to estimate the value of intermediate positions.
So it becomes the way we have now that the agent relies on heuristic evaluation functions in non-terminal nodes to approximate long-term outcomes and guide the search.
