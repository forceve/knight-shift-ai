    ...
    def _load_tests_from_disk(self):
        for json_file in self.tests_dir.glob("*.json"):
            try:
                ...
                # Reset status if it was queued or running (these states don't persist across restarts)
                if data.get("status") in ("queued", "running"):
                    # If completed, mark as completed; otherwise mark as running to allow restart
                    if data.get("completed", 0) >= (data.get("total_games") or data.get("games", 0)):
                ...
                print(f"Error loading test from {json_file}: {e}")
                continue
