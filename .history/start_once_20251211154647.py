"""
Simple one-shot starter for Knight Shift AI.
Starts backend once, then frontend once, opens browser, and waits.
No lock files, no restart loops.

Usage:
    python start_once.py
"""
...
def main():
    if not os.path.isdir(BACKEND_DIR) or not os.path.isdir(FRONTEND_DIR):
        print("Error: backend/ or frtend/ directory not found next to this script.")
