...
class UltimateEngine(BaseEngine):
    """
    Strongest built-in engine: iterative deepening with improved evaluation and move ordering.
    """
    ...
    def evaluate(self, board: chess.Board) -> int:
        ...
        for piece_type, value in PIECE_VALUES.items():
            ...
            score += len(w) * value
            score -= len(b) * value
            pst = self._pst_for_piece(piece_type, endgame=self._is_endgame(board))
            if pst:
                for sq in w:
                    score += pst[sq]
                for sq in b:
        ...
        # Mobility
        score += 3 * len(list(board.legal_moves)) if board.turn == chess.WHITE else -3 * len(list(board.legal_moves))

        # Pawn structure: passed, isolated, doubled
        score += self._pawn_structure(board)

        # King safety: castled bonus, open file penalties near king
    ...
    def _pawn_structure(self, board: chess.Board) -> int:
        ...
        for color in (chess.WHITE, chess.BLACK):
            ...
            for sq in pawns:
                ...
                score += 5 * adv if color == chess.WHITE else -5 * adv
            # Isolated / doubled
            for file_idx, count in enumerate(files):
                ...
                if count == 1 and left == 0 and right == 0:
                    score += -20 if color == chess.BLACK else 20
        return score
    ...
    def order_moves(self, board: chess.Board, moves):
        # Prioritize PV (not tracked), then captures, then checks, then killer/historic placeholder.
